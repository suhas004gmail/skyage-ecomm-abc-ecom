      ...
      <div className="flex flex-col md:flex-row items-start gap-0 h-[560px]">
        <form onSubmit={handleSubmit} className="w-full md:w-1/2 h-full bg-white p-6 rounded shadow space-y-4">
          ...
          <div>
            ...
            <input value={name} name="name" onChange={e=>setName(e.target.value)} className="mt-1 block w-full border rounded px-3 py-2" required />
          </div>
          <div>
            <label className="block text-sm font-medium">Enter Product/Service Name</label>
            <input value={productService} name="productService" onChange={e=>setProductService(e.target.value)} className="mt-1 block w-full border rounded px-3 py-2" required />
          </div>
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="block text-sm font-medium">{t('mobileNumber')}</label>
              <input value={mobile} name="mobile" onChange={e=>setMobile(e.target.value)} type="tel" className="mt-1 block w-full border rounded px-3 py-2" required />
            </div>
            <div className="flex-1">
              <label className="block text-sm font-medium">{t('email')}</label>
              <input value={email} name="email" onChange={e=>setEmail(e.target.value)} type="email" className="mt-1 block w-full border rounded px-3 py-2" required />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium">{t('message')}</label>
            <textarea value={message} name="message" onChange={e=>setMessage(e.target.value)} className="mt-1 block w-full border rounded px-3 py-2" rows={5} required />
          </div>
